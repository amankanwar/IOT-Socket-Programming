...
int main(int argc, char *argv[])
{
    ...
    int pidNumber;   // to check for the PID of the process
...
if((socketFD_new = accept(socketFD, (struct sockaddr *)&clientAddr, &clientLength)) < 0)
{
   ...
   /* here we will implement our fork, wherein this given process
    * will have another child process to handel any incoming message
    * from the client. While the parent process will be used to
...
if(pidNumber == 0)  // here we are in the child proces 
{
...
if(pidNumber < 0){printf("\nIssure creating a new child process\n"); exit(1);}
